...
async function applyIndexOnCreate(requestedIndex) {
  ...
  const conflict = await Aircraft.findOne({ index: idx }).lean();
  if (conflict) {
    // Move the conflicting one to the new end (which is total + 1, since we're about to add a new doc)
    await Aircraft.findByIdAndUpdate(conflict._id, { $set: { index: total + 1 } });
  }
  return idx;
}
...
async function applyIndexOnUpdate(docId, requestedIndex) {
  ...
  const current = await Aircraft.findById(docId).lean();
  if (!current) throw new Error("Aircraft not found");

  const oldIndex = Number(current.index) || null;
  if (oldIndex === idx) {
    return idx; // no-op
  }

  const conflict = await Aircraft.findOne({ index: idx, _id: { $ne: docId } }).lean();
  if (conflict) {
    // Swap: conflict takes oldIndex if exists, else send conflict to last position
    if (ensurePositiveInt(oldIndex)) {
      await Aircraft.findByIdAndUpdate(conflict._id, { $set: { index: oldIndex } });
    } else {
